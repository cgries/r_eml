...
title <- "Long-term studies of secondary succession and community assembly in the prairie-forest ecotone of eastern Kansas, Hay meadow restoration experiment"
pubDate <- "2016"
abstract <- as(set_TextType("emlobfs1/E2Abstract.docx"), "abstract")
...
#add methods
methods <- set_methods("emlobfs1/E2Methods.docx")
...
# write.csv(attributes, file = "emlobfs1/E2_1metadata.csv", row.names = FALSE)
...
attributes <- read.csv("emlobfs1/E2_1metadata.csv", header = TRUE, sep = ",", quote = "\"", as.is = TRUE, na.strings = "")

factors <- read.csv("emlobfs1/E2_1factors.csv", header = TRUE, sep = ",", quote = "\"", as.is = TRUE)
...
physical <- set_physical("E2 Plant Biomass 6 16.csv", 
                         numHeaderLines = "1", 
                         recordDelimiter = "\\r\\n",
                         url = "https://foster.ku.edu/sites/foster.ku.edu/files/files/E2%20Plant%20Biomass%206%2016.csv")


#pull to gether information for the dataTable
dataTable1 <- new("dataTable",
                  entityName = "E2 Plant Biomass 6 16.csv",
...
# #second data table
# df <- read.csv("emlobfs1/E2_Plant_Species_Composition_6_16_long.csv", header=TRUE, sep=",", quote="\"", as.is = TRUE, na.strings = "NA")
# 
# #set up the attribute metadata csv file
# rows <- ncol(df)
# 
...
# attributes$columnClasses[attributes$columnClasses == "integer"] <- "numeric"
# #write the prepared template to a csv file
# write.csv(attributes, file = "emlobfs1/E2_2metadata.csv", row.names = FALSE)

#read the attributes file back in with all new entries
attributes <- read.csv("emlobfs1/E2_2metadata.csv", header = TRUE, sep = ",", quote = "\"", as.is = TRUE, na.strings = "")

factors <- read.csv("emlobfs1/E2_2factors.csv", header = TRUE, sep = ",", quote = "\"", as.is = TRUE)
...
#with the attributes and factors data frames in place we can create the attributeList element 
attributeList <- set_attributes(attributes, factors = factors, col_classes = col_classes)
...
dataTable2 <- new("dataTable",
                  entityName = "E2_Plant_Species_Composition_6_16_long.csv",
                  entityDescription = "This file contains data on plant species composition (live biomass of each component species expressed in g m-2) harvested from each of the 32 experimental 10 x10 m subplots (2001-2015).",
                  physical = physical,
                  attributeList = attributeList)
...
write_eml(eml, "emlobfs1/edi2.xml")
